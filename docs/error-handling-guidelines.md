...
The fwupd update step (in both scripts) also uses this pattern, via `fwupd_classify_update <rc> <output>` (`ok` / `dbx_blocked` / `fail`), for a slightly different reason: fwupd's exit code is reliable (0 ok, 2 "nothing to do"), but the output text is needed to recognize one known failure mode — fwupd refusing a UEFI revocation-database (dbx) update because a boot binary still in the EFI System Partition is on the new revocation list. `dbx_blocked` still records `FAIL fwupd (update) (exit <rc>)` (with a `(dbx blocked — see hint above)` suffix, following `pk_note_for`'s suffix precedent — the keyword prefix stays intact) and still flips the exit code; the classification only adds two `warn` lines telling the operator the host-side fix (reinstall the distro's shim/grub packages to refresh the ESP copies, reboot, re-run). Do not soften it to OK/SKIP (the dbx security update is genuinely still pending) and do not make the script remediate it (rewriting ESP boot binaries is not an updater's job). Its dbx match is deliberately a *bare substring* (`is present in dbx`), unlike the anchored pkcon patterns: fwupd embeds the phrase mid-line, and a false match here can only add the hint — both `dbx_blocked` and `fail` report FAIL, so it can never turn a failure into a false OK.
